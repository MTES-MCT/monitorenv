...
context('Axe core RGAA check that ', () => {
  ...
  it('mission list should respect RGAA criteria', () => {
    visitSideWindow()
    cy.wait('@getMissions')
    cy.injectAxe()
    // @ts-ignore
  ...
  it('a mission form with surveillance action opened should respect RGAA criteria', () => {
    visitSideWindow()
    cy.wait('@getMissions')
  ...
  it('a mission form with a control action opened should respect RGAA criteria', () => {
    visitSideWindow()
    cy.wait('@getMissions')
    cy.getDataCy('edit-mission-34').scrollIntoView().click({ force: true })
  ...
  it('a mission form with attached reporting should respect RGAA criteria', () => {
    visitSideWindow()
    cy.wait('@getMissions')
    cy.injectAxe()
    cy.getDataCy('edit-mission-34').scrollIntoView().click({ force: true })
  ...
  it('a vigilance area form should respect RGAA criteria', () => {
    cy.visit('/#@-192242.97,5819420.73,9.93')
    cy.wait(['@getAmps', '@getRegulatoryAreas', '@getVigilanceAreas'])
    cy.clickButton('Arbre des couches')
  ...
  it('vigilance area list should respect RGAA criteria', () => {
    visitSideWindow()
  ...
  it('reporting list should respect RGAA criteria', () => {
    visitSideWindow()
  ...
  it('dashboard list should respect RGAA criteria', () => {
    visitSideWindow()
    cy.clickButton('Tableaux de bord')
    cy.wait(['@getDashboards', '@getRegulatoryAreas', '@getAmps'])
    cy.injectAxe()
    // @ts-ignore
    cy.checkA11y(null, {
      runOnly: {
  ...
  it('a dashboard form should respect RGAA criteria', () => {
    visitSideWindow()
    cy.clickButton('Tableaux de bord')
    cy.wait(['@getDashboards', '@getRegulatoryAreas', '@getAmps'])
