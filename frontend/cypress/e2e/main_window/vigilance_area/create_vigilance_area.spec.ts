...
describe('Create Vigilance Area', () => {
  ...
  it('Should successfully create a vigilance area', () => {
    ...
    cy.clickButton('Ajouter une autre source externe')
    cy.fill('Nom', 'Mike data')
    cy.fill('Numéro de téléphone', '0123456789')
    cy.fill('Email', 'mike.data@gmail.com')
    cy.get("textarea[name='sourceComments']").type('Source ajoutée via e2e')
    cy.clickButton('Valider')

    cy.clickButton('Ajouter une source CACEM')
    cy.fill('Nom', 'Document OFB')
    cy.fill('Lien', 'www.ofb.fr/document')
    cy.get("textarea[name='sourceComments']").type('Document interne CACEM')
    cy.clickButton('Valider')
    ...
    cy.wait('@createVigilanceArea').then(({ request, response }) => {
      ...
      expect(createdVigilanceArea.sources).to.have.length(3)
      expect(createdVigilanceArea.sources[0].controlUnitContacts[0].id).equal(2)
      expect(createdVigilanceArea.sources[0].type).equal('CONTROL_UNIT')
      ...
      expect(createdVigilanceArea.sources[1].comments).equal('Source ajoutée via e2e')
      expect(createdVigilanceArea.sources[1].type).equal('OTHER')
      expect(createdVigilanceArea.sources[2].name).equal('Document OFB')
      expect(createdVigilanceArea.sources[2].link).equal('www.ofb.fr/document')
      expect(createdVigilanceArea.sources[2].comments).equal('Document interne CACEM')
      expect(createdVigilanceArea.sources[2].type).equal('INTERNAL')
