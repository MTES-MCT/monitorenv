  ...
  useEffect(() => {
    ...
    if (selectedMission) {
      ...
      const hasAlreadyAMissionFeature =
        ...
        (currentFeatureOver.id.includes(`${Layers.MISSIONS.code}:${selectedMission.id}`) ||
          currentFeatureOver.id.includes(`${Layers.MISSION_TO_ATTACH_ON_REPORTING.code}:${selectedMission.id}`))
