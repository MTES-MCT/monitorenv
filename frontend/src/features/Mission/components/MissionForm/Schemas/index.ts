...
const MissionTypesSchema = Yup.array()
  .of(Yup.mixed<Mission.MissionTypeEnum>().oneOf(Object.values(Mission.MissionTypeEnum)).required())
  .ensure()
...
const EnvActionNoteSchema: Yup.SchemaOf<Mission.EnvActionNote> = Yup.object()
  .shape({
    actionStartDateTimeUtc: Yup.string().required('Requis'),
    actionType: Yup.mixed().oneOf([Mission.ActionTypeEnum.NOTE]),
    id: Yup.string().required()
  })
  .required()

const NewEnvActionSchema = Yup.lazy((value, context) => {
  if (value.actionType === Mission.ActionTypeEnum.CONTROL) {
    return getNewEnvActionControlSchema(context)
  }
  if (value.actionType === Mission.ActionTypeEnum.SURVEILLANCE) {
    return getNewEnvActionSurveillanceSchema(context)
  }
  if (value.actionType === Mission.ActionTypeEnum.NOTE) {
...
const NewMissionSchema: Yup.SchemaOf<Mission.NewMission> = Yup.object()
  .shape({
