...
import { cancelCreateAndRedirectToFilteredList } from '../../../useCases/cancelCreateAndRedirectToFilteredList'
...
export function ControlUnitWarningMessage({ controlUnitIndex }: { controlUnitIndex: number }) {
  ...
  const message = useMemo(() => {
    ...
    if (engagedControlUnit.missionSources.length === 1) {
      ...
      return `Une autre mission, ouverte par le ${Mission.missionSourceEnum[source].label}, est en cours avec cette unité. `
    }

    if (engagedControlUnit.missionSources.length > 1) {
      return `D'autres missions en cours, ouvertes par le ${engagedControlUnit.missionSources
        .map(source => Mission.missionSourceEnum[source].label)
