...
export function ActionForm({ currentActionIndex, setCurrentActionIndex }: ActionFormProps) {
  ...
  const [envActionsField, , envActionsHelper] = useField<Mission.EnvAction[]>('envActions')
  const envActionIndex = (envActionsField.value ?? []).findIndex(envAction => envAction.id === currentActionIndex)
  const [actionTypeField] = useField<Mission.ActionTypeEnum>(`envActions.${envActionIndex}.actionType`)
  const [actionIdField] = useField<Mission.EnvAction['id']>(`envActions.${envActionIndex}.id`)
