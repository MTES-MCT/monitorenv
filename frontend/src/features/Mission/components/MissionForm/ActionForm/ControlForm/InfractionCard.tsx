...
export function InfractionCard({
  ...
  const [, meta] = useField<Mission.Infraction>(infractionPath)
  const [targetTypeField] = useField<Mission.EnvActionControl['actionTargetType']>(
  ...
  const [vesselType] = useField<Mission.VesselTypeEnum>(`${infractionPath}.vesselType`)
  const [registrationNumber] = useField<Mission.Infraction['registrationNumber']>(
    `${infractionPath}.registrationNumber`
  )
  const [controlledPersonIdentity] = useField<Mission.Infraction['controlledPersonIdentity']>(
    `${infractionPath}.controlledPersonIdentity`
  )
  const [companyName] = useField<Mission.Infraction['companyName']>(`${infractionPath}.companyName`)
  const [infractionType] = useField<Mission.InfractionTypeEnum>(`${infractionPath}.infractionType`)
  const [formalNotice] = useField<Mission.FormalNoticeEnum>(`${infractionPath}.formalNotice`)
  const [natinf] = useField<Mission.Infraction['natinf']>(`${infractionPath}.natinf`)
  ...
  switch (infractionType?.value) {
    case undefined:
      ...
      break
    case Mission.infractionTypeLabels.WITHOUT_REPORT.code:
      libelleInfractionType = Mission.infractionTypeLabels.WITHOUT_REPORT.libelle
      break
    case Mission.infractionTypeLabels.WITH_REPORT.code:
      libelleInfractionType = Mission.infractionTypeLabels.WITH_REPORT.libelle
      break
    case Mission.infractionTypeLabels.WAITING.code:
    default:
      libelleInfractionType = Mission.infractionTypeLabels.WAITING.libelle
  }
    ...
    <Wrapper $hasError={!!meta.error}>
      <Summary>
        {targetTypeField.value === TargetTypeEnum.VEHICLE && (
          <VehicleType>
            ...
            {vehicleTypeField?.value === VehicleTypeEnum.VESSEL
              ? ` – ${Mission.vesselTypeLabel[vesselType?.value] ?? 'Type non défini'}`
              : ''}
            &nbsp;&ndash;&nbsp;
          </VehicleType>
        )}
        ...
        <SummaryDetails>
          ...
          {formalNotice?.value === Mission.FormalNoticeEnum.YES && <Info accent={Accent.PRIMARY}>MED</Info>}
