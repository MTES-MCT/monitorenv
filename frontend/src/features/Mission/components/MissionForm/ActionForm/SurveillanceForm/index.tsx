...
export function SurveillanceForm({ currentActionIndex, remove, setCurrentActionIndex }) {
  ...
  } = useFormikContext<Mission.Mission<Mission.EnvActionSurveillance>>()

  const { actionsMissingFields } = useMissionAndActionsCompletion()

  const [actionsFields] = useField<Mission.EnvAction[]>('envActions')
  ...
  ) as FormikErrors<Mission.EnvActionSurveillance>
  ...
  const surveillances = actionsFields.value.filter(action => action.actionType === Mission.ActionTypeEnum.SURVEILLANCE)
  ...
  useEffect(() => {
    if (
      actionsMissingFields[currentActionIndex] === 0 &&
      currentAction?.completion === Mission.CompletionStatus.TO_COMPLETE
    ) {
      setFieldValue(`envActions[${envActionIndex}].completion`, Mission.CompletionStatus.COMPLETED)
    ...
    if (
      actionsMissingFields[currentActionIndex] > 0 &&
      currentAction?.completion === Mission.CompletionStatus.COMPLETED
    ) {
      setFieldValue(`envActions[${envActionIndex}].completion`, Mission.CompletionStatus.TO_COMPLETE)
    }
