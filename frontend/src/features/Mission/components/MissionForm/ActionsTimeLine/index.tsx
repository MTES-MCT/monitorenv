...
export function ActionsTimeLine({ currentActionIndex, setCurrentActionIndex }) {
  ...
  const { errors, setFieldValue, values } = useFormikContext<Partial<Mission.Mission | Mission.NewMission>>()

  const envActions = values?.envActions as Mission.EnvAction[]
  ...
  const isFirstSurveillanceAction = !envActions?.find(
    action => action.actionType === Mission.ActionTypeEnum.SURVEILLANCE
  )
  ...
  const sortedActions: Array<Mission.ActionsTypeForTimeLine> = useMemo(
    () =>
      actions &&
  ...
  const handleAddControlAction = useCallback(() => {
    const newControl = actionFactory({ actionType: Mission.ActionTypeEnum.CONTROL })
  ...
  const handleAddNoteAction = useCallback(() => {
    const newNote = actionFactory({ actionType: Mission.ActionTypeEnum.NOTE })
