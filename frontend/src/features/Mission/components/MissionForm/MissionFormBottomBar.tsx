...
export function MissionFormBottomBar({
  ...
  const allowEditMission =
    ...
    values?.missionSource === Mission.MissionSourceEnum.MONITORENV ||
    values?.missionSource === Mission.MissionSourceEnum.MONITORFISH

  const allowDeleteMission = !missionIsNewMission && allowEditMission

  const isFromMonitorFish = values.missionSource === Mission.MissionSourceEnum.MONITORFISH
    ...
    <Footer>
      ...
      <MissionInfos>
        <span>
          ...
          {values?.createdAtUtc && (
            <>
              Mission créée par le {Mission.missionSourceEnum[values?.missionSource]?.label} le{' '}
