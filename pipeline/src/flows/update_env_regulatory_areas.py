...
@task
def extract_regulatory_areas_hashes() -> pd.DataFrame:
    """
...
def update_regulatory_areas(new_regulatory_areas: pd.DataFrame):
    ...
    with e.begin() as connection:
        ...
        logger.info(
            f"Updating regulatory_areas from temporary table {len(new_regulatory_areas)}"
        )
        connection.execute(
            text(
...
@task
def load_regulatory_areas(new_regulatory_areas: pd.DataFrame):
    """Load the output of ``extract_new_regulatory_areas`` task into ``regulatory_areas``
    table.
...
def update_env_regulatory_areas_flow():
    ...
    if cond_insert is True:
        new_regulatory_areas = extract_cacem_regulatory_areas(ids_to_insert)
