...
def test_get_control_unit_ids(expected_env_actions, expected_control_unit_ids):
    ids = get_control_unit_ids(expected_env_actions)
...
def test_extract_control_units(
    ...
    units = extract_control_units(control_unit_ids=expected_control_unit_ids)
...
def test_extract_all_control_units(
    ...
    units = extract_all_control_units()
    units["email_addresses"] = units.email_addresses.map(sorted)
...
def test_render(sample_control_unit_actions):
    ...
    with open(
        TEST_DATA_LOCATION / "emails/expected_rendered_email.html", "w"
    ) as f:
        f.write(html)
...
@patch("src.flows.email_actions_to_units.send_email")
@patch("src.flows.email_actions_to_units.sleep")
def test_send_env_actions_email(
    ...
    sent_messages = send_env_actions_email(
