...
from src.flows.themes_natinfs import associate_themes_natinfs, extract_natinf, extract_themes, load_themes_natinfs, natinfs_themes_flow
...
def themes() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "theme": ["Police des mouillages", "Police des mouillages", "Especes protegees", "Theme sans sous-themes" ],
            "subtheme": [
...
def test_flow_themes_natinfs(reset_test_data):
    ...
    assert len(themes_natinfs) == 0
    state = natinfs_themes_flow(return_state=True)
    assert state.is_completed()
    themes_natinfs = read_query(query = "SELECT * FROM themes_natinfs", db="monitorenv_remote")
    ## I was expecting 17 themes_natinfs but we also map older themes (ended_at < now()) so there are 17 * 2 = 34
    assert len(themes_natinfs) == 34