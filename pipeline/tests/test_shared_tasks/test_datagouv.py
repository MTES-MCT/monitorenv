...
def test_api_url():
    assert (
        api_url("/some/path/") == "https://www.data.gouv.fr/api/1/some/path/"
    )
...
def test_get_csv_file_object():
    ...
    file_object = get_csv_file_object(input_df)
...
def test_get_geopackage_file_object_with_layers():
    ...
    file_object = get_geopackage_file_object(input_gdf, layers=layers)

    assert isinstance(file_object, BytesIO)

    layer_1, layer_2 = ["a", "b"]
    assert gpd.list_layers(file_object)["name"].values.tolist() == [
        layer_1,
        layer_2,
    ]

    file_object.seek(0)
    gdf_from_file_object = gpd.read_file(
        file_object, driver="GPKG", layer=layer_1
    )
    pd.testing.assert_frame_equal(
    ...
    file_object.seek(0)
    gdf_from_file_object = gpd.read_file(
        file_object, driver="GPKG", layer=layer_2
    )
    pd.testing.assert_frame_equal(
        gdf_from_file_object,
