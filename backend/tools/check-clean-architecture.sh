...
readonly SRC_PATH="../src/main/kotlin/fr/gouv/cacem/monitorenv"
...
readonly UNAUTHORIZED_PACKAGES_USAGE_COUNT_IN_USE_CASES=$(find "${DOMAIN_USE_CASES_PACKAGE_PATH}" \( -name "*.kt" -o -name "*.java" \) -exec grep -Ewl "${UNAUTHORIZED_PACKAGES_USAGE_IN_DOMAIN}" {} + | wc -l)
