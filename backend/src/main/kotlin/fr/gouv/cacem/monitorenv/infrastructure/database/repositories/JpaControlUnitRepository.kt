...
class JpaControlUnitRepository(
    ...
    override fun findNearbyUnits(geometry: Geometry): List<NearbyUnit> {
        val missions = dbMissionRepository.findAllByGeometry(geometry).map {
            it.toMissionEntity(mapper)
        }
        return missions.flatMap { mission -> mission.controlUnits.map { controlUnit -> controlUnit to mission } }
