...
abstract class BaseVesselModel(
    ...
    fun toVessel(
        additionalInformation: VesselAdditionalInformationModel? = null,
        files: List<VesselFileModel> = emptyList(),
        legalStatusLabel: String? = null,
        nafLabel: String? = null,
    ): VesselEntity =
        VesselEntity(
            additionalInformation = additionalInformation?.toVesselAdditionalInformation(),
            files = files.map { it.toVesselFile() },
            id = id,
            batchId = batchId,
            shipId = shipId,
            status = status,
