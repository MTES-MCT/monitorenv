...
class SSEReporting {
    ...
    fun handleUpdateMissionEvent(event: UpdateReportingEvent) {
        logger.info("SSE: Received reporting event for mission ${event.reporting.reporting.id}.")
        val reportingId = event.reporting.reporting.id

        logger.info("SSE: Sending update of mission $reportingId to ${sseStore.size} listener(s).")
