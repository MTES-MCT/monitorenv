...
SELECT
    ...
    infractions->>'id' AS infraction_id,
    infractions->'natinf' AS natinf,
    CASE
        WHEN infractions->>'infractionType' = 'WITH_REPORT' THEN 'PV'
        WHEN infractions->>'formalNotice' = 'YES' THEN 'MED'
        WHEN infractions->>'infractionType' = 'WITHOUT_REPORT' THEN 'RAR'
        ELSE NULL
    END AS infraction_type,
...
FROM env_actions a
LEFT JOIN LATERAL jsonb_array_elements(a.value->'infractions') AS infractions ON TRUE
