...
class ControlPlansControllerITests {
    ...
    fun `Should get all control plan themes`() {
        // Given
        val controlPlanTheme1 = ControlPlanThemeEntity(
            id = 1,
            theme = "Theme Police des mouillages",
        )
        val controlPlanTheme2 = ControlPlanThemeEntity(
            id = 2,
            theme = "Theme Protection des espèces",
        )
        val controlPlanSubTheme1 = ControlPlanSubThemeEntity(
            id = 10,
            themeId = 1,
            subTheme = "SubTheme ZMEL",
            year = 2024,
        )
        val controlPlanTag = ControlPlanTagEntity(
            id = 100,
            themeId = 2,
            tag = "Tag Bichique",
        )
        val controlPlan = Triple(
        ...
        mockMvc.perform(get("/bff/v1/control_plans/2024"))
            ...
            .andExpect(jsonPath("$.subThemes[\"10\"].subTheme").value(controlPlanSubTheme1.subTheme))
            .andExpect(jsonPath("$.subThemes[\"10\"].themeId").value(controlPlanSubTheme1.themeId))
