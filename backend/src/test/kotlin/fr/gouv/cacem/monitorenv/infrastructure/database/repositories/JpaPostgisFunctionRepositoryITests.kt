...
class JpaPostgisFunctionRepositoryITests : AbstractDBTests() {
    ...
    fun `normalizeMultipolygon Should return normalized multipolygon`() {
        // Given
        val polygonThatCrosses180lineButOk =
            WKTReader().read(
        ...
        val polygonThatCrosses180lineButNotOk =
            WKTReader().read(
        ...
        val normalizedMultipolygon =
            jpaPostgisFunctionRepository.normalizeMultipolygon(
    ...
    fun `normalizeGeometry Should return normalized geometry`() {
        ...
        val expectedPointOutOfBoundsNormalized =
            WKTReader().read(
                "POINT(-7.053858716342802 47.482021414897076)",
            ) as Point
        // When
        val untouchedMultipolygon = jpaPostgisFunctionRepository.normalizeMultipolygon(polygonThatCrosses180lineButOk)
        val normalizedMultipolygon =
